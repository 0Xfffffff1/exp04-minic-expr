...
{
    // 识别产生式：logicalAndExp: equalityExp (T_AND equalityExp)*;
    if (ctx->T_AND().empty()) {
...
{
    // 识别产生式：equalityExp: relationalExp ((T_EQ | T_NEQ) relationalExp)*;
    if (ctx->T_EQ().empty() && ctx->T_NEQ().empty()) {
        return visitRelationalExp(ctx->relationalExp()[0]);
    }

    ast_node *left, *right;
    auto eqOps = ctx->T_EQ();
    auto neqOps = ctx->T_NEQ();
    int totalOps = eqOps.size() + neqOps.size();
...
{
    // 识别产生式：relationalExp: addExp ((T_LT | T_GT | T_LE | T_GE) addExp)*;
    ...
    auto leOps = ctx->T_LE();
    auto geOps = ctx->T_GE();
    int totalOps = ltOps.size() + gtOps.size() + leOps.size() + geOps.size();
...
{
    // 识别文法产生式：unaryOp: T_SUB | T_NOT;
    if (ctx->T_SUB()) {
        return ast_operator_type::AST_OP_UNARY_MINUS;
    } else if (ctx->T_NOT()) {
        return ast_operator_type::AST_OP_LOGICAL_NOT;
    }
    return ast_operator_type::AST_OP_MAX;
}
